...
describe('getIpAddress', () => {
  ...
  test('normalizes a full IPv6 address to compressed form', () => {
    expect(
      getIpAddress(headers({ 'cf-connecting-ip': '2001:0db8:0000:0000:0000:0000:0000:0001' })),
    ).toBe('2001:db8::1');
  });

  test('converts an IPv4-mapped IPv6 address to IPv4', () => {
    expect(getIpAddress(headers({ 'cf-connecting-ip': '::ffff:1.2.3.4' }))).toBe('1.2.3.4');
  });

  test('strips the port from an IPv4 address with a port', () => {
    expect(getIpAddress(headers({ 'cf-connecting-ip': '1.2.3.4:8080' }))).toBe('1.2.3.4');
  });

  test('returns garbage input unchanged as a fallback', () => {
    expect(getIpAddress(headers({ 'cf-connecting-ip': 'not-an-ip' }))).toBe('not-an-ip');
  });
